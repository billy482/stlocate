...
*  Last modified: Tue, 09 Jul 2013 22:33:09 +0200                         *
\*************************************************************************/
 ...
 */
static int sl_string_valid_utf8_char(const char * string);
...
bool sl_string_check_valid_utf8(const char * string) {
	...
	while (*ptr) {
		int size = sl_string_valid_utf8_char(ptr);
...
void sl_string_fix_invalid_utf8(char * string) {
	...
	while (*ptr) {
		...
		char * ptr_end = ptr + 1;
		while (*ptr_end && sl_string_valid_utf8_char(ptr_end) == 0)
			ptr_end++;
